...
import type { ReportData } from '@/app/builder/data/reportData';
import { createNewNodeTemplate } from '@/app/builder/data/nodeTemplates';
import { createNewReportTemplate } from '@/app/builder/data/reportTemplates';
...
export default function Home() {
  ...
  // Load workflow data from URL parameter (from generator)
  useEffect(() => {
    ...
    if (dataParam) {
      try {
        ...
        const parsed = JSON.parse(decoded) as WorkflowData
        setWorkflowData(parsed)
        // Clear URL parameter after loading
        const newUrl = window.location.pathname
        window.history.replaceState({}, '', newUrl)
      } catch (error) {
        console.error('Error loading workflow data from URL:', error)
        alert('Failed to load workflow data from URL')
      }
    }
  }, [])
  ...
  const [generateReportDialogState, setGenerateReportDialogState] = useState<{
    open: boolean;
  }>({
    open: false
  })
  ...
  // Generate reports from workflow
  const generateReportsFromWorkflow = () => {
    if (workflowData.nodes.length === 0) {
      alert("No nodes found in workflow. Please add nodes first.");
      return;
    }

    // Filter nodes that have fields (exclude validation nodes and other non-form nodes)
    const nodesWithFields = workflowData.nodes.filter(node =>
      node.fields && Array.isArray(node.fields) && node.fields.length > 0
    );

    if (nodesWithFields.length === 0) {
      alert("No nodes with fields found in workflow. Please add form nodes with fields first.");
      return;
    }

    const reports: ReportData[] = nodesWithFields.map((node, index) => {
      const baseReport = createNewReportTemplate();

      // Clean node name: lowercase, spaces to underscore, remove symbols like dash/dot
      ...
      // Clean workflow name: lowercase, spaces to underscore, remove symbols
      const cleanWorkflowName = workflowData.name
        .toLowerCase()
        .replace(/\s+/g, '_')  // Replace spaces with underscore
        .replace(/[-\.\/\\:;,!@#$%^&*()+=\[\]{}|'"<>?]/g, '') // Remove symbols
        .replace(/_+/g, '_'); // Replace multiple underscores with single underscore
      ...
      return {
        ...baseReport,
        _id: `${baseReport._id}_${node.id}`,
        key: `report_${cleanNodeName}`,
        name: `Report ${node.name}`,
        description: `Report ${node.name}`,
        fields: node.fields
          .filter(field => field.isVisible !== false) // Only include fields that are visible (undefined or true)
          .map((field, fieldIndex) => ({
            ...field,
            order: fieldIndex,
            // Add report-specific properties
            aggregateShow: true,
          })),
        triggers: [`${workflowData._id}.${node.id}`],
      };
    });

    setGeneratedReports(reports);
    setGenerateReportDialogState({ open: true });
  }

  const openGenerateReportDialog = () => {
    generateReportsFromWorkflow();
  }

  const closeGenerateReportDialog = () => {
    setGenerateReportDialogState({ open: false });
    setGeneratedReports([]);
  }

  const exportGeneratedReports = () => {
    if (generatedReports.length === 0) return;

    if (generatedReports.length === 1) {
      // Single report - download as report.json
      const report = generatedReports[0];
      const blob = new Blob([JSON.stringify(report, null, 2)], { type: 'application/json' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `${report.key}.json`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
    } else {
      // Multiple reports - download as zip would be ideal, but for now download individually
    ...
    alert(`Successfully exported ${generatedReports.length} report(s)!`);
    closeGenerateReportDialog();
  }

  const openGeneratedReportInNewTab = (report: ReportData) => {
    // Open report in new tab with the report data
    const reportUrl = `/reports?data=${encodeURIComponent(JSON.stringify(report))}`;
  ...
  const DraggableField = ({
    ...
    // Determine if this is a subfield or main field
    const isSubField = level > 0;
    const dragType = isSubField ? ItemTypes.SUBFIELD : ItemTypes.FIELD;
    ...
    const [{ isDragging }, drag] = useDrag({
      ...
      item: {
        index: fieldIndex,
        nodeId,
        parentPath: isSubField ? parentPath : undefined,
        level
      },
      collect: (monitor) => ({
    ...
    const [, drop] = useDrop({
      accept: dragType, // Only accept the same type (FIELD or SUBFIELD)
      hover: (item: { index: number, nodeId: string, parentPath?: string, level: number }) => {
        if (!ref.current) return
        if (item.nodeId !== nodeId) return

        // For subfields, ensure they're in the same parent
        if (isSubField && item.parentPath !== parentPath) return
        // For main fields, ensure both are main fields (level 0)
        if (!isSubField && item.level !== 0) return
    ...
    return (
      <div ref={ref} className={`ml-${level * 4}`}>
        <div
          className={`${isSubField
            ? 'bg-gray-700 border-l-4 border-blue-400'
            : 'bg-gray-800'
            } p-3 rounded-lg group relative transition-all duration-200 hover:bg-gray-750 ${isDragging ? 'opacity-50 scale-95' : 'opacity-100 scale-100'
            } min-h-[60px] flex flex-col justify-center cursor-move ${isSubField ? 'ml-2' : ''
            }`}
        >
          {/* Drag handle */}
          <div
            ...
            title={isSubField ? 'Drag to reorder sub-fields only' : 'Drag to reorder main fields only'}
          >
            <GripVertical className={`w-4 h-4 ${isSubField ? 'text-blue-400' : 'text-gray-500'}`} />
          </div>
              ...
              onClick={(e) => {
                ...
                if (confirm(`Remove field "${field.label}"?`)) {
                  if (parentPath) {
                    ...
                    setWorkflowData(prev => ({
                      ...
                      nodes: prev.nodes.map(node => {
                        ...
                        const updatedFields = node.fields.map(f => {
                          if (f.key === parentKey && f.fields) {
                            // Remove the sub-field and reorder
                            const filteredSubFields = f.fields.filter(sf => sf.key !== field.key);
                            const reorderedSubFields = filteredSubFields.map((sf, index) => ({
                              ...sf,
                              order: index + 1
                            }));
                            return { ...f, fields: reorderedSubFields };
                          }
                          return f;
                        });
                    ...
                    const node = workflowData.nodes.find(n => n.id === nodeId);
                    if (node) {
                      const updatedFields = node.fields.filter(f => f.key !== field.key);
        ...
        {hasSubFields && !minimizedFields[fieldPath] && (
          <div className="ml-4 mt-2 space-y-2 border-l-2 border-gray-600 pl-4">
            ...
            {field.fields.map((subField: any, subIndex: number) => (
              <DraggableField
                key={`${fieldPath}-subfield-${subField.key}-${subIndex}`} // More unique key
                field={subField}
                fieldIndex={subIndex}
                nodeId={nodeId}
              ...
              <>
                ...
                <div className="space-y-3 w-full">
                  {node.fields.map((field, fieldIndex) => (
                    <DraggableField
                      key={`${node.id}-field-${field.key}-${fieldIndex}`} // More unique key
                      field={field}
  ...
  return (
    <DndProvider backend={HTML5Backend}>
      <div className="font-sans flex flex-col items-center justify-center min-h-screen pb-20 gap-16 lg:p-20 lg:pt-30 p-8 w-full">
            ...
            <>
              <div className="flex justify-between items-center w-full">
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="outline" onClick={openGenerateReportDialog} disabled={workflowData.nodes.length === 0}>
                    Generate Reports
                  </Button>
                  <Button variant="outline" onClick={() => {
        ...
        <Dialog open={generateReportDialogState.open} onOpenChange={closeGenerateReportDialog}>
          <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Generated Reports</DialogTitle>
              <DialogDescription>
                Generated {generatedReports.length} report(s) from workflow "{workflowData.name}"
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4">
                  ...
                  <div key={report._id} className="border rounded-lg p-4 bg-gray-50">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">Key: {report.key}</p>
                      </div>
                      <div className="flex gap-2">
                        <button
                          onClick={() => {
                        ...
                        <button
                          onClick={() => openGeneratedReportInNewTab(report)}
                          className="px-3 py-1 text-xs bg-blue-500 text-white rounded hover:bg-blue-600"
                        >
                          Open in Report Canvas
                        </button>
                      </div>
                    </div>

                    <div className="text-sm">
                      <p className="text-gray-600">Fields: {report.fields.length}</p>
                      <div className="flex flex-wrap gap-1 mt-1">
                        {report.fields.slice(0, 5).map((field) => (
                          <span key={field.key} className="px-2 py-1 bg-blue-100 text-blue-800 rounded text-xs">
                            {field.label}
                          </span>
                        ))}
              ...
              <DialogFooter className="flex justify-end gap-2 pt-4 border-t">
                <button
                  onClick={closeGenerateReportDialog}
                  className="px-4 py-2 text-gray-600 border border-gray-300 rounded hover:bg-gray-50"
                >
                  Cancel
                </button>
                <button
                  onClick={exportGeneratedReports}
